...
const emit = defineEmits<{ select: [message: OpenInboxMessage] }>()
...
const preview = computed(() => props.message.body.replace(/\s+/g, ' ').trim())
</script>

<template>
  <button type="button" class="open-inbox-item" :class="{ 'open-inbox-item--unread': !message.isRead, 'open-inbox-item--selected': selected }" @click="emit('select', message)">
    ...
    <span class="open-inbox-item__copy">
      ...
      <b>{{ message.title || 'Sebuah catatan kecil untukmu' }}</b>
      <small>{{ preview }}</small>
    </span>
    <span class="open-inbox-item__state"><MailOpen v-if="message.isRead" :size="14" /><Mail v-else :size="14" />{{ message.isRead ? 'Dibaca' : 'Baru' }}</span>
  </button>
</template>
